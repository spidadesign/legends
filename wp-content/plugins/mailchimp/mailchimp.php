		...
		if (is_array($opt_val) && isset($opt_val['area'])) {
			...
			if ($var['req'] == 'Y') {
				if (empty($opt_val['addr1']) || empty($opt_val['city'])) {
					$errs[] = sprintf(__("Please enter your email address.", 'mailchimp_i18n'), esc_html($var['name']));
					$success = false;
				}
			}
			...
			if (!$retval) {
				switch($api->errorCode) {
					...
					case '250' :
						...
						$errs[] = sprintf(__("Please enter your email address.", 'mailchimp_i18n'), esc_html($mv_tag_keys[$field]['name']));
						break;
					case '254' :
